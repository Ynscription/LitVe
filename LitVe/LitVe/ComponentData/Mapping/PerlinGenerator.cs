...
namespace LitVe.ComponentData.Mapping {
	class PerlinGenerator {
		...
		public PerlinGenerator (float pers, float freq, float amp, int oct, int se) {
			persistence = pers;
			frequency = freq;
			amplitude = amp;
			octaves = oct;
		...
		private float Total(float i, float j) {
		...
		private float Noise (int x, int y) {
			...
			double r = 1.0 - t * 0.931322574615478515625e-9;// 1073741824.0);
			return (float)r;
		}
